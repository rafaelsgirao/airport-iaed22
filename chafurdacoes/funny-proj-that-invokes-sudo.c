...
/*			ordered_airport_store[j] = index in the store of airport  */
...
{
    int m = (right + left) / 2;
	fprintf(stderr, "DEBUG: right=%d,left=%d\n", right, left);
	if (right - left < 20) {
		fprintf(stderr, "DEBUG: bubble sort executed\n");
		bubbleSortFlights(flights, left, right, departures);
		return;
	}
    if (right <= left)
        return;
...
/*		if (date_comparison > 0 || (date_comparison == 0 && time_comparison > 0) || i == m + 1)*/
		if(compareFlights(flight_j, flight_i, departures) == 1) {
...
/*
 departures should be passed as 1 if we're sorting an airport's
 departures façade, 0 otherwise (sorting arrivals).
*/
void bubbleSortFlights(int flights[], int left, int right, int departures) {
	/*
...
void addFlight() {
	...
	/*Arrival date is only different if flight arrives on next day's morning
		(max duration = 12h)*/
...
/*Read input and parse for errors.*/
void parseFlight(Flight *flight) {
	int i;
	Date tmp_date = system_date;
	Flight *flight_i;
	int arprt_departure_i, arprt_arrival_i;
	...
	for(i=0;i<flight_count;i++) {
		...
		if(!strcmp(flight_i->code, flight->code)) {
			if(!(compareDate(flight->departure_date, flight_i->departure_date))) {
	...
	/*if departure date is in the future relative to tmp_date
	 * (compareDate(...) < 0) , then it's invalid*/
	tmp_date.year++;
	...
	if(!(MIN_FLIGHT_CAP <= flight->capacity &&
		 flight->capacity <= MAX_FLIGHT_CAP)) {
...
/*		mergeSortFlights(arprt->departures, 0, arprt->departure_count, 1);*/
		bubbleSortFlights(arprt->departures, 0, arprt->departure_count, 1);
		arprt->departure_dirty = 0;
		/*FIXME*/
...
/*		mergeSortFlights(arprt->arrivals, 0, arprt->arrival_count, 0); */
		bubbleSortFlights(arprt->arrivals, 0, arprt->arrival_count, 0);
		arprt->arrival_dirty = 0;
/*		airports[arprt_i] = arprt;*/
	}
...
void setDate() {
	...
	if (date.day <= 0 || date.month <= 0 || date.year <= 0 ||
		compareDate(system_date, date) < 0) {
		printf("invalid date\n");
		return;
	}
